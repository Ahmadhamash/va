...
export class AuthService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    const envMasterKey = process.env.API_MASTER_KEY?.trim();
    let displayKey: string;
    let isNewKey = false;

    if (count === 0) {
      // Use configured key when deployed with another service that needs API access.
      displayKey =
        envMasterKey ||
      ...
      if (envMasterKey) {
        const keyHash = this.hashKey(envMasterKey);
        const configuredKey = await this.apiKeyRepository.findOne({ where: { keyHash } });
        if (!configuredKey) {
          await this.seedApiKey(envMasterKey, 'Configured Master Key', ApiKeyRole.ADMIN);
          displayKey = envMasterKey;
          isNewKey = true;
          try {
            writeFileSync(API_KEY_FILE, envMasterKey, 'utf-8');
          } catch (err) {
            this.logger.warn('Could not save configured API key file', { error: String(err) });
          }
        }
      }
    }
    ...
    this.logger.log('');
    const safeDisplayKey = displayKey.startsWith('owa_k1_')
      ? `${displayKey.substring(0, 12)}... (hidden)`
      : displayKey;
    ...
    this.logger.log(`     ${safeDisplayKey}`);
    this.logger.log('');
    this.logger.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    this.logger.log('');
  }
