...
from models import (
    Booking, BusinessPolicy, ChatSession, DeliveryRule, Escalation,
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "in your answer, or there is a complaint. "
...
- **escalate_to_human**: Call this when:
  ...
  When you escalate, tell the customer politely that you are transferring them
  to a team member who can help better.
...
async def _exec_get_catalog(func_args: dict, user_id: uuid.UUID, db: AsyncSession) -> dict:
    ...
    # Eagerly load variants for the matched items
    if rows:
        item_ids = [r.id for r in rows]
        variant_stmt = select(ItemVariant).where(ItemVariant.item_id.in_(item_ids))
        variant_rows = list((await db.execute(variant_stmt)).scalars().all())
        variant_map: dict[uuid.UUID, list[ItemVariant]] = {}
        for v in variant_rows:
            variant_map.setdefault(v.item_id, []).append(v)
        for r in rows:
            r.variants = variant_map.get(r.id, [])
...
async def _exec_escalate(
    ...
    return {
        ...
        "message": "The conversation has been escalated to a human agent. "
                   "Please inform the customer that a team member will follow up shortly.",
    }
