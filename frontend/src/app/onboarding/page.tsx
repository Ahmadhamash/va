...
export default function OnboardingPage() {
  ...
  async function runTest() {
    if (user) {
      ...
      form.set("message", testMessage);
      setTestBusy(true);
      try {
        const result = await apiRequest<{ reply: string }>("/chat/send", {
          method: "POST",
          body: form
        });
        setTestReply(result.reply);
        setNotice("Backend agent test completed through /api/chat/send.");
        return;
      } catch (error) {
        setNotice(error instanceof Error ? error.message : "Backend agent test failed.");
      } finally {
        setTestBusy(false);
      }
    }
    ...
    <div key="test" className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <Button onClick={runTest} disabled={testBusy}>{testBusy ? "جاري الاختبار..." : "جرّب الرد"}</Button>
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
