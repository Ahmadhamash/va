...
export default function Navbar() {
  ...
  useEffect(() => {
    if (user?.role === "client") {
      const poll = () => api.get("/handoff/", { params: { status: "pending" } }).then(({ data }) => {
  ...
  const links = user?.role === "admin" ? [{ to: "/", label: t("nav_clients") }] : [
    ...
    { to: "/handoff", label: t("nav_handoff"), badge: pendingCount },
    { to: "/ai-safety", label: t("nav_ai_safety") },
    { to: "/automation", label: t("nav_automation") },
    { to: "/voice-settings", label: t("nav_voice_settings") },
