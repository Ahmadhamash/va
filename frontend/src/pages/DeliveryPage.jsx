  ...
  return <div className="py-7" dir="rtl">
    <div className="app-container space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-black text-gray-950 dark:text-white sm:text-2xl">{t("txt_43")}</h1>
        <button onClick={() => {
        resetForm();
        setShowForm(true);
      }} className="btn-primary">{t("txt_44")}</button>
      </div>
      ...
      {loading ? <p className="text-gray-500">{t("txt_26")}</p> : rules.length === 0 ? <div className="bg-brand-50 border border-brand-100 text-brand-700 text-sm rounded-lg px-4 py-3">{t("txt_57")}</div> : <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {rules.map(rule => <div key={rule.id} className="surface p-5 space-y-2">
