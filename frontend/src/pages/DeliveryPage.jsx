...
export default function DeliveryPage() {
  ...
  async function loadRules() {
    setLoading(true);
    try {
      const { data } = await api.get("/delivery-rules");
      setRules(data);
    } finally {
      setLoading(false);
    }
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 space-y-6" dir="rtl">
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="مثل: عمان، الزرقاء، إربد"
                required
                dir="auto"
                className={inputCls}
              />
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, free_above: e.target.value })
                }
                placeholder="مثل: 50"
                className={inputCls}
              />
          ...
          <label className="flex items-center gap-2 text-sm text-gray-700">
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, pickup_available: e.target.checked })
              }
            />
            استلام من المحل متاح
          </label>
      ...
      {loading ? (
        ...
        <div className="bg-brand-50 border border-brand-100 text-brand-700 text-sm rounded-lg px-4 py-3">
          لم تضف أي مناطق توصيل بعد. أضف مناطقك حتى يتمكن المساعد الذكي من
          إخبار الزبائن عن تكاليف ومدة التوصيل.
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {rules.map((rule) => (
            ...
            >
              ...
              <div className="text-sm text-gray-600 space-y-1">
                ...
                {rule.free_above && (
                  <p>
                    🎁 مجاني فوق {rule.free_above} {rule.currency}
                  </p>
                )}
                {rule.estimated_days && <p>🕐 {rule.estimated_days}</p>}
                {rule.pickup_available && <p>📍 استلام من المحل متاح</p>}
                {rule.notes && (
