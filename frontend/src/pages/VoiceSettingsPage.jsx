...
export default function VoiceSettingsPage() {
  ...
  const selectedVoice = voices.find((v) => v.value === settings?.preferred_voice);
  ...
  function chooseProvider(nextProvider) {
    ...
    setSettings((prev) => ({
      ...
      preferred_voice: firstVoice?.value || (nextProvider === "openai" ? "nova" : ""),
      tts_config:
  ...
  async function previewVoice(voice = selectedVoice) {
    setMsg("");
    setPreviewUrl("");
    setPreviewing(voice?.value || settings.preferred_voice || "selected");
    try {
      const ttsConfig =
        provider === "elevenlabs"
          ? configForVoice(voice, settings.tts_config)
          : settings.tts_config || {};
      const { data } = await api.post("/voice-settings/preview", {
        tts_provider: provider,
        preferred_voice: voice?.value || settings.preferred_voice,
